...
out_dir = f'out/diffdim/{init_method}/R_{num_repeat}-S_{true_cluster_size}'
print(out_dir)
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

for num_centroids in range(5,9,4):
    # True labels
    ...
    for dim in tqdm(tot_dims):
        ...
        for i in range(num_repeat):
            ...
            true_points = np.concatenate(
                [rng.multivariate_normal(mean, sigma * cov, size=true_cluster_size) for mean in centroids])
        ...
        # Final points
            ...
            lloydL1_his = lloydL1(points, centroids_input=copy.deepcopy(centroids), k=num_centroids)
            lloydL1_centroids, lloydL1_labels = lloydL1_his['centroids'], lloydL1_his['labels']
            kmed_his = kmed(points, centroids_input=copy.deepcopy(centroids), k=num_centroids)
            kmed_centroids, kmed_labels = kmed_his['centroids'], kmed_his['labels']
            kmeans_his = kmeans(points,centroids_input=copy.deepcopy(centroids),k=num_centroids)
            kmeans_centroids, kmeans_labels = kmeans_his['centroids'], kmeans_his['labels']

            # plot training difference between centroids
            if i ==0:
                plot_diff_i(lloydL1_his, kmed_his, kmeans_his, i,
                            title = f'K_{num_centroids}-D_{dim}-{i}th_repeat', out_dir = f'{out_dir}/repeats')
