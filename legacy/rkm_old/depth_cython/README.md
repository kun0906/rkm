...
(Generate 'TukeyDepth.cpython-310-darwin.so' under './build')
```

# Load xxx.so and call the function
```python
# import the module
import numpy as np
import ctypes
...
lib_file = glob.glob('build/*/TukeyDepth*.so')[0]
...
f = lib.depthTukey2dExact_PY(1, 2.0, nrows, ncols, z)
print(f"f:{f}")
