...
extern "C" {
    double depthTukey2dExact_PY(double U, double V, int nrows, int ncols, double A[nrows][ncols])
    {
       std::cout << U << "\n";
//       std::cout << sizeof(*A) << "\n";
//       std::cout << &A << "\n";
//       size_t m = sizeof(A[0]);    // number of columns
//       size_t n = sizeof(*A)/m;     // number of rows
//       std::cout << m << ", " << n << "\n";
       ...
       // convert array to arma::mat
       // https://stackoverflow.com/questions/65868229/convert-a-2d-array-into-an-armadillo-matrix-mat-object
       arma::mat B(&A[0][0], nrows, ncols);
       return Tukey::depthTukey2dExact(U, V, B);
    }
}

// Instruction to compile and link to generate 'xxx.so'
//  #g++ -c -std=c++11 -fPIC TukeyDepth.cpp -o TukeyDepth.o
//  #g++ -shared -o libTukeyDepth.so TukeyDepth.o -larmadillo
