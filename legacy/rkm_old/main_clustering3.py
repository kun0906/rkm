...
def main(num_repeat=5, radius=3, noise_cov=5, prop=0.80, initial_method='omnisicent'):
    true_cluster_size = 100

    for num_centroids in range(5, 35 + 1, 5):
        # True labels
        ...
        for dim in tqdm(tot_dims):
            ...
            for i in range(num_repeat):
                ...
                outliers = np.random.multivariate_normal(np.zeros(dim), np.eye(dim) * noise_cov,
        ...
        out_dir = f'out/R_{num_repeat}-D_{dim}-K_{num_centroids}-r_{radius}-cov_{noise_cov}-pxK_{prop}/{initial_method}'
...
def main_call(args):
    try:
        num_repeat, radius, noise_cov, prop, initial_method = args[0], args[1], args[2], args[3], args[4]
        print(f'num_repeat: {num_repeat}, radius: {radius}, noise_cov: {noise_cov}, prop: {prop}, '
              f'initial_method: {initial_method}', flush=True)
        main(num_repeat, radius, noise_cov, prop, initial_method)
...
if __name__ == '__main__':
    ...
    noise_covs = [1, 9, 25, 100, 250,] # [1, 9, 25, 100, 250]
    props = [0.05, 0.1, 0.25, 0.4, 0.5]
        ...
        for radius in radiuses:  #:
            for noise_cov in noise_covs:
                for prop in props:
                    ...
                    params_lst.append((num_repeat, radius, noise_cov, prop, initial_method))
