...
for num_centroids in range(4,9,6):
    ...
    for sigma_out in tqdm(sigma_out_vec):
        ...
        for i in range(num_repeat):
            ...
            outliers = rng.multivariate_normal(np.ones(dim)*0,
                                           np.eye(dim)*sigma_out**2, size = math.floor(true_single_cluster_size * prop))
    ...
    plt.plot(sigma_out_vec, kmeans_misc_avg, '-', label='Llyod (k_means)',color="blue")
    plt.errorbar(sigma_out_vec, kmeans_misc_avg, yerr=kmeans_misc_err, fmt='none', ecolor='black', capsize=3)

    if is_sc:
        plt.plot(sigma_out_vec, sc_lloydL1_misc_avg, '-o', label='SC-Lloyd-$L_1$', color="lightgreen")
        ...
        plt.plot(sigma_out_vec, sc_kmeans_misc_avg, '-s', label='SC-Llyod (k_means)', color="skyblue")
    ...
    plt.plot(sigma_out_vec, robust_sc_kmeans_misc_avg, '-p', label='RSC-Llyod (k_means)', color="steelblue")
