...
# for num_centroids in range(2, 10, 10):
    ...
    for rad_out in tqdm(rad_out_vec):
        ...
        for i in range(num_repeat):
            ...
            lloydL1_centroids, lloydL1_labels = k_medians_l2(points, centroids_input=init_centroids,
            ...
            if is_rsc:
                ...
                shape_ = lloydL1_labels.shape
                robust_sc_lloydL1_centroids, robust_sc_lloydL1_labels = np.zeros((num_centroids, dim)), np.zeros(shape_)
                robust_sc_kmed_centroids, robust_sc_kmed_labels = np.zeros((num_centroids, dim)), np.zeros(shape_)
                robust_sc_kmeans_centroids, robust_sc_kmeans_labels = np.zeros((num_centroids, dim)), np.zeros(shape_)
    ...
    if is_rsc:
        ...
        plt.errorbar(rad_out_vec, robust_sc_kmed_misc_avg, yerr=robust_sc_kmed_misc_err, fmt='none', ecolor='black', capsize=3)
