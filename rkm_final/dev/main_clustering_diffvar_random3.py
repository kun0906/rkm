...
for num_centroids in [2]: #range(5,9,4):
    # True labels
    ...
    for sigma_out in tqdm(sigma_out_vec):
        ...
        for i in range(num_repeat):
            ...
            #
            # centroids *= radius
            # centroids = [np.ones((dim, ))*5, np.ones((dim,))*(-5)]
            centroids = np.asarray([np.asarray([50, 0]), np.asarray([-50, 0])])
            ...
            true_points = np.concatenate(
                [rng.multivariate_normal(mean,  cov*(sigma**2), size=true_cluster_size) for mean in centroids], axis=0)
            ...
            print(i, 'lloydL1_centroids:, ', lloydL1_centroids, ' ', dict(sorted(collections.Counter(lloydL1_labels).items())))
            print(i, 'kmed_centroids: ', kmed_centroids, dict(sorted(collections.Counter(kmed_labels).items())))
            print(i, 'kmeans_centroids: ', kmeans_centroids, dict(sorted(collections.Counter(kmeans_labels).items())))
