...
if without_alignment:
    from clustering_random import *
else:
    from clustering_random2 import *
...
out_dir = f'out/diffvar/{init_method}/R_{num_repeat}-S_{true_cluster_size}'
print(out_dir)
...
for num_centroids in range(2,9,9):
    # True labels
    ...
    for sigma_out in tqdm(sigma_out_vec):
        ...
        # Final points
            ...
            labels = np.concatenate((true_labels, np.ones((len(outliers, )))*num_centroids))
            ## Without outliers
            # points = true_points

            # Perform k-means clustering with k clusters
            if without_alignment:
                lloydL1_centroids, lloydL1_labels = lloydL1(points, centroids_input=init_centroids, k=num_centroids,
                ...
                lloydL1_centroids, lloydL1_labels = lloydL1(points,centroids_input=init_centroids,k=num_centroids, true_centroids=centroids, true_labels=labels)
                kmed_centroids, kmed_labels = kmed(points,centroids_input=init_centroids,k=num_centroids, true_centroids=centroids, true_labels=labels)
                kmeans_centroids, kmeans_labels = kmeans(points,centroids_input=init_centroids,k=num_centroids, true_centroids=centroids, true_labels=labels)
            # lloyd_order_centroids,lloyd_order_labels = lloyd_order(points,centroids_input=copy.deepcopy(centroids),k=num_centroids)
            # print(lloydL1_labels)
            #
            # print(true_labels)
