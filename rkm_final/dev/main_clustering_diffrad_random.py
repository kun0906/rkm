...
dim = 10
rad_out_vec = np.trunc(np.linspace(0, 100, 10))

init_method = 'random'
out_dir = f'out/diffrad/{init_method}/R_{num_repeat}'

if not os.path.exists(out_dir):
    os.makedirs(out_dir)

for num_centroids in [5]: #range(5, 10, 6):
    # True labels
    ...
    for rad_out in tqdm(rad_out_vec):
        ...
        for i in range(num_repeat):
            ...
            true_points = np.concatenate(
                [r.multivariate_normal(mean, np.identity(dim) * sigma ** 2, size=true_cluster_size) for mean in
                 centroids])
            ...
            centroids_out_dir = r.multivariate_normal(np.zeros(dim), np.eye(dim), size=1)

            centroids_out_dir /= np.linalg.norm(centroids_out_dir, axis=1)[:, np.newaxis]

            outlier_mean = rad_out / np.sqrt(dim) * centroids_out_dir[0]

            outliers = r.multivariate_normal(outlier_mean, np.eye(dim) * sigma_out ** 2,
                                             size=math.floor(true_cluster_size * prop))
            ...
            kmedL1_acd.append(np.median((kmedL1_centroids - centroids) ** 2))
    ...
    data = {'outlier rad': rad_out_vec, 'kmedians misc': kmed_misc_avg, 'kmedians misc err_bar': kmed_misc_err,
    ...
    plt.plot(rad_out_vec, kmed_misc_avg, '-.', label='Lloyd-$L_1$', color="green")
    plt.errorbar(rad_out_vec, kmed_misc_avg, yerr=kmed_misc_err, fmt='none', ecolor='black', capsize=3)

    plt.plot(rad_out_vec, kmedL1_misc_avg, '--', label='k-median', color="purple")
    plt.errorbar(rad_out_vec, kmedL1_misc_avg, yerr=kmedL1_misc_err, fmt='none', ecolor='black', capsize=3)
    ...
    plt.plot(rad_out_vec, kmed_acd_avg, '-.', label='Lloyd-$L_1$', color="green")
    plt.errorbar(rad_out_vec, kmed_acd_avg, yerr=kmed_acd_err, fmt='none', ecolor='black', capsize=3)

    plt.plot(rad_out_vec, kmedL1_acd_avg, '--', label='k-median', color="purple")
    plt.errorbar(rad_out_vec, kmedL1_acd_avg, yerr=kmedL1_acd_err, fmt='none', ecolor='black', capsize=3)
...
print(kmedL1_centroids)
#
# print(np.sum((kmed_centroids - centroids) ** 2) / num_centroids)
#
# print(np.sum((kmedL1_centroids - centroids) ** 2) / num_centroids)
print("\n")

df_describe_kmed = pd.DataFrame(kmed_acd)

df_describe_kmedL1 = pd.DataFrame(kmed_acd)

print(df_describe_kmed.describe())
print("\n")
print(df_describe_kmedL1.describe())
# print("\n")
# print(len(kmed_acd>5)/len(kmed_acd))
# print("\n")
# print(len(kmedL1_acd>5)/len(kmedL1_acd))
